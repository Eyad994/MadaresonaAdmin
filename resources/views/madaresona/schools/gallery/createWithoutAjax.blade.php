...
@section('content')
    <div class="container">
        ...
        <div class="row">
            ...
            @foreach($gallery as $item)
                <div class="col-md-3 galleryDeleteDiv_{{ $item->id }} form-group">
                    <label class="btn_close btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow galleryDelete"
                           id="{{ $item->id }}" style="color: #262673 !important;">
                        <i class="fa fa-times text-muted" style="color: red !important;"></i>
                    </label>
                    <img class="img-gallery" src="{{ asset('/images/'.$schoolName.'/gallery/'.$item->img) }}"
        ...
        <br>
        <div class="row">
            <form id="galleryForm" action="{{ route('submitGallery') }}" method="POST" enctype="multipart/form-data"
                  style="width: 100%" onsubmit="false">
                @csrf
